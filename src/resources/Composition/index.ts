...
export class Composition extends ResourceMain implements ResourceMaster {
   ...
   * @returns 
   */
 async getDiagnosisFromEnconter(diagnosis:ENCOUNTER["diagnosis"], index:number, diagnosisStringArray:string[]){
  if(!diagnosis || index >= diagnosis.length ){
    return;
  }

  const resource = this.getFromMultResource({"reference" : diagnosis[index].condition.reference})
