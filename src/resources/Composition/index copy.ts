...
import { Condition, GcpFhirCRUD, GcpFhirSearch } from "../..";
...
export class Composition extends ResourceMain implements ResourceMaster {
  ...
  getFHIR(options: COMPOSITOIN) {
    ...
    const getHtmlText = async(): Promise<string> => {
   ...
   * @returns 
   */
 async getDiagnosisFromEnconter(encounter:ENCOUNTER):Promise<string[]>{
  let ret:string[]=[]
  if(encounter?.diagnosis  && encounter.diagnosis.length > 0){
      encounter.diagnosis.forEach(async el=>{
      const id = this.getFromMultResource({"reference" : el.condition.reference}).id
      ret.push((await new GcpFhirCRUD().getFhirResource(id, el.condition.type)).data.text.div)
    })
  }
  return ret;
 }
